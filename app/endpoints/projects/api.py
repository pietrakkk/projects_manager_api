...
from endpoints.projects.schema import (GeoJsonfileType, ProjectCreate,
...
@router.post(
    "/", response_model=ProjectListResponse, status_code=status.HTTP_201_CREATED
)
async def create_project(
    ...
    if geojson_content.get("type") != GeoJsonfileType.feature.value:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid GeoJSON format"
        )

    if not geojson_content.get("geometry", []):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No valid geometry found"
        )

    return await project_service.create_project(
        payload=payload, file_content=geojson_content, file_name=file.filename
    )
...
async def get_project(project_id: str):
    try:
        ...
        raise HTTPException(
            ...
            detail="Cannot download project",
        )
